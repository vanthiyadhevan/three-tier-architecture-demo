pipeline {
    ...
    stages {
        ...
        stage('Build Docker & Catalog') {
            steps {
                sh 'docker build -f ./catalog/Dockerfile -t ${DOCKER_REPO_NAME}/${DOCKER_CATELOG}:${BUILD_ID} ./catalog'
            }
        }
        ...
        stage('Build Docker & Load Gen') {
            steps {
                sh 'docker build -f ./loadgen/Dockerfile -t ${DOCKER_REPO_NAME}/${DOCKER_LOAD_GEN}:${BUILD_ID} ./loadgen'
            }
        }
        ...
        stage('Build Docker & Rating') {
            steps {
                sh 'docker build -f ./rating/Dockerfile -t ${DOCKER_REPO_NAME}/${DOCKER_RATING}:${BUILD_ID} ./rating'
            }
        }
